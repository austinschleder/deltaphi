...
player_db.set_tiers(['qb', 'rb', 'wr', 'te'], LEAGUE_SIZE, MINIMUM_GAMES_PLAYED)
print(player_db)

league1 = fs.League(players=player_db.get_player_pool(ROSTER_SLOTS), teams=fs.create_teams(LEAGUE_SIZE, NICKNAMES), seasons=[])
print(league1)

[print(p) for p in sorted(league1.get_position_players('wr'), key=lambda x: x.position_rank)]
